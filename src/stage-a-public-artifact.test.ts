  ...
  test("every exported subpath executes and shares one SQLite provenance owner", () => {
    ...
    try {
      ...
      writeFileSync(probe, `
        ...
        const savedDatabaseQuery = owned.query;
        const savedStatement = owned.query("SELECT 1 AS value");
        const savedStatementGet = savedStatement.get;

        const accessorReceivers = [];
        let containerGetterReceiver;
        let containerSetterReceiver;
        Object.defineProperty(contracts.TODOS_CONTRACTS, "stageAReceiverProbe", {
          configurable: true,
          get() {
            containerGetterReceiver = this;
            return this;
          },
          set(_value) {
            containerSetterReceiver = this;
          },
        });
        const containerGetterResult = contracts.TODOS_CONTRACTS.stageAReceiverProbe;
        contracts.TODOS_CONTRACTS.stageAReceiverProbe = "probe";
        accessorReceivers.push(
          ["container-get", containerGetterReceiver, contracts.TODOS_CONTRACTS],
          ["container-set", containerSetterReceiver, contracts.TODOS_CONTRACTS],
          ["container-result", containerGetterResult, contracts.TODOS_CONTRACTS],
        );
        const guardedIterator = root.ACCESS_PROFILES.values();
        let iteratorGetterReceiver;
        let iteratorSetterReceiver;
        Object.defineProperty(guardedIterator, "stageAReceiverProbe", {
          configurable: true,
          get() {
            iteratorGetterReceiver = this;
            return this;
          },
          set(_value) {
            iteratorSetterReceiver = this;
          },
        });
        const iteratorGetterResult = guardedIterator.stageAReceiverProbe;
        guardedIterator.stageAReceiverProbe = "probe";
        accessorReceivers.push(
          ["iterator-get", iteratorGetterReceiver, guardedIterator],
          ["iterator-set", iteratorSetterReceiver, guardedIterator],
          ["iterator-result", iteratorGetterResult, guardedIterator],
        );
        let untrustedCode = "";
        ...
        const databaseCapabilityDenials = [
          ["database-direct", () => owned.query("SELECT 1")],
          ["database-saved-method", () => Reflect.apply(savedDatabaseQuery, owned, ["SELECT 1"])],
          ["statement-direct", () => savedStatement.get()],
          ["statement-saved-method", () => Reflect.apply(savedStatementGet, savedStatement, [])],
        ].map(([label, operation]) => ({ label, denied: authorityDenied(operation) }));
        const accessorModeDenials = [
          ["container-get", () => contracts.TODOS_CONTRACTS.stageAReceiverProbe],
          ["container-set", () => { contracts.TODOS_CONTRACTS.stageAReceiverProbe = "remote"; }],
          ["iterator-get", () => guardedIterator.stageAReceiverProbe],
          ["iterator-set", () => { guardedIterator.stageAReceiverProbe = "remote"; }],
        ].map(([label, operation]) => ({ label, denied: authorityDenied(operation) }));
        ...
        console.log(JSON.stringify({
          ...
          databaseCapabilityDenials,
          accessorReceiverIdentity: accessorReceivers.map(([label, receiver, publicValue]) => ({
            label,
            matches: receiver === publicValue,
          })),
          accessorModeDenials,
      ...
      expect(payload.databaseCapabilityDenials).toEqual([
        "database-direct",
        "database-saved-method",
        "statement-direct",
        "statement-saved-method",
      ].map((label) => ({ label, denied: true })));
      expect(payload.accessorReceiverIdentity).toEqual([
        "container-get",
        "container-set",
        "container-result",
        "iterator-get",
      ...
      expect(payload.accessorModeDenials).toEqual([
        "container-get",
        "container-set",
        "iterator-get",
        "iterator-set",
