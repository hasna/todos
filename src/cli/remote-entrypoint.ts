...
interface TodosCliAuthorityClient {
  readonly baseUrl: string;
}

export interface TodosCliAuthorityDependencies {
  readonly getCloudClient: (env: Env) => TodosCliAuthorityClient | null;
}

const DEFAULT_AUTHORITY_DEPENDENCIES: TodosCliAuthorityDependencies = {
  getCloudClient: getTodosCloudClient,
};
...
export function initializeTodosCliAuthority(
  ...
  dependencies: TodosCliAuthorityDependencies = DEFAULT_AUTHORITY_DEPENDENCIES,
  ...
  const client = dependencies.getCloudClient(env);
  if (!client) {
