    ...
    try {
      ...
      if (dry_run) {
        return {
          content: [{
            type: "text" as const,
            text: `[dry-run] target=${target} delay=${effectiveDelay}ms\n\n${message}`,
          }],
        };
      }
    ...
    try {
      ...
      if (dry_run) {
        return {
          content: [{
            type: "text" as const,
            text: `[dry-run] list=${taskList.name} target=${target} tasks=${tasks.length} delay=${effectiveDelay}ms\n\n${message}`,
          }],
        };
      }

      const dispatch = createDispatch({
        title: `Task list: ${taskList.name}`,
        task_list_id: resolvedListId,
        target_window: target,
        message,
        delay_ms: effectiveDelay,
        scheduled_at,
  ...
  async ({ targets, task_ids, task_list_id, stagger_ms, delay_ms, dry_run }) => {
    try {
      ...
      return {
        content: [{
          type: "text" as const,
          text: `Dispatched to ${dispatches.length} target(s):\n${lines.join("\n")}`,
        }],
      };
    } catch (e) {
      return { content: [{ type: "text" as const, text: formatError(e) }], isError: true };
  ...
  ({ status, limit }) => {
    try {
      ...
      if (dispatches.length === 0) {
        return { content: [{ type: "text" as const, text: "No dispatches found." }] };
      }
