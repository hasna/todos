...
describe("MCP token utilities", () => {
  ...
  it("supports profile groups and additive tool groups", () => {
    ...
    expect(shouldRegisterToolForProfile("todos_cloud_status", "agent", undefined)).toBe(false);
    expect(shouldRegisterToolForProfile("todos_cloud_status", "agent", "cloud")).toBe(true);
  ...
  it("appends token telemetry only when enabled", () => {
    const response = { content: [{ type: "text" as const, text: "hello world" }] };
    expect(withMcpTokenTelemetry(response, "get_task", false)).toBe(response);

    const instrumented = withMcpTokenTelemetry(response, "get_task", true) as typeof response;
    expect(instrumented.content[0]!.text).toContain("[mcp-token-telemetry tool=get_task");
  });
});
