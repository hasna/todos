...
const MCP_GROUP_DEFS: Array<{ id: string; name: string; description: string; match: (tool: string) => boolean }> = [
  ...
  },
  {
    ...
    match: (t) =>
      /dependenc|relationship|graph|critical_path|blocked|ready_tasks|sync_kg|find_path|impact/.test(t),
  },
  {
    id: "import_export",
    name: "Import and export",
  ...
  },
  {
    ...
    match: (t) =>
      /backup|restore|migration|activity|schedule|release_check|env_snapshot|topology|run_record/.test(t),
  },
  {
    id: "security",
    name: "Security",
