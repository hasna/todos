...
describe("findBestAgent", () => {
  ...
  it("should return agent with role=agent", () => {
    const agent = registerAgent({ name: "janus", role: "agent", status: "active" }, db);
    const result = findBestAgent({ title: "Test" } as any, db);
    expect(result).toBe("janus");
  });

  it("should skip admin and observer role agents", () => {
    registerAgent({ name: "adminone", role: "admin", status: "active" }, db);
    registerAgent({ name: "observerone", role: "observer", status: "active" }, db);
    expect(findBestAgent({ title: "Test" } as any, db)).toBeNull();
  });
...
describe("autoAssignTask", () => {
  ...
  it("should assign via capability match", async () => {
    const agent = registerAgent({ name: "frontenddev", role: "agent", status: "active", capabilities: ["frontend", "react"] }, db);
    ...
    expect(result.agent_name).toBe("frontenddev");
    expect(result.method).toBe("capability_match");
  });

  it("should assign to least busy agent when no capability match", async () => {
    const a1 = registerAgent({ name: "busyagent", role: "agent", status: "active" }, db);
    const a2 = registerAgent({ name: "freeagent", role: "agent", status: "active" }, db);
    ...
    const result = await autoAssignTask(task.id, db);
    expect(result.agent_name).toBe("freeagent");
