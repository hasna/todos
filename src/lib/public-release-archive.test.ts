...
describe("packed public release byte scanner", () => {
  ...
  test("checks path collisions with bounded near-linear work below the entry cap", () => {
    const entryCount = 4_096;
    const archive = fixture([
      validReadme(),
      ...Array.from({ length: entryCount }, (_value, index) => ({
        path: `package/dist/generated-${index.toString().padStart(5, "0")}.js`,
    ...
    Map.prototype.get = function patchedGet<Key, Value>(this: Map<Key, Value>, key: Key): Value | undefined {
      indexOperations += 1;
      return originalMapGet.call(this, key) as Value | undefined;
    };
    Map.prototype.has = function patchedHas<Key, Value>(this: Map<Key, Value>, key: Key): boolean {
      indexOperations += 1;
      return originalMapHas.call(this, key);
    };
    Map.prototype.set = function patchedSet<Key, Value>(this: Map<Key, Value>, key: Key, value: Value): Map<Key, Value> {
      indexOperations += 1;
      return originalMapSet.call(this, key, value) as Map<Key, Value>;
    };
    Map.prototype.keys = function patchedKeys<Key, Value>(this: Map<Key, Value>): MapIterator<Key> {
      indexOperations += 1;
      const iterator = originalMapKeys.call(this) as MapIterator<Key>;
      return {
        next(): IteratorResult<Key> {
          const result = iterator.next();
          if (!result.done) indexOperations += 1;
          return result;
        },
        [Symbol.iterator]() { return this; },
      } as MapIterator<Key>;
    };
    Set.prototype.add = function patchedAdd<Value>(this: Set<Value>, value: Value): Set<Value> {
      indexOperations += 1;
      return originalSetAdd.call(this, value) as Set<Value>;
    };
    Set.prototype.has = function patchedSetHas<Value>(this: Set<Value>, value: Value): boolean {
      indexOperations += 1;
      return originalSetHas.call(this, value);
    };
    try {
      expect(readPackedRegularFiles(archive)).toHaveLength(entryCount + 1);
    } finally {
    ...
    // The prior all-path scan performed roughly n^2 / 2 yielded-key operations.
    // The path index now performs O(total path segments) Map/Set work and stays
    // under this deterministic, timing-independent operation bound.
    expect(indexOperations).toBeLessThan((entryCount + 1) * 16);
  });
