...
export type TodosPostgresSnapshotRecordType =
  | "tasks"
  | "projects"
...
export interface CreatePostgresTodosSnapshotStoreOptions {
  service?: string;
...
export interface PullPostgresTodosSnapshotOptions {
  since?: string;
  objectTypes?: TodosPostgresSnapshotRecordType[];
}

export interface PostgresTodosSnapshotPushResult {
  records: number;
  objectTypes: Partial<Record<TodosPostgresSnapshotRecordType, number>>;
}

export interface TodosPostgresSnapshotRecordRow {
  object_type: TodosPostgresSnapshotRecordType;
  object_id?: string;
  payload: unknown;
...
export const DEFAULT_TODOS_POSTGRES_RECORD_TABLE = "todos_records";
export const DEFAULT_TODOS_POSTGRES_CURSOR_TABLE = "todos_cursors";

export function postgresTodosSchemaSql(
  tableName = DEFAULT_TODOS_POSTGRES_RECORD_TABLE,
...
export class PostgresTodosSnapshotStore {
  ...
  constructor(
    ...
    options: CreatePostgresTodosSnapshotStoreOptions = {},
  ) {
  ...
  async ensureSchema(): Promise<void> {
    for (const sql of postgresTodosSchemaSql(this.tableName, this.cursorTableName)) {
  ...
  async pushSnapshot(
  ...
  ): Promise<PostgresTodosSnapshotPushResult> {
    ...
    const existing = await this.client.query<TodosPostgresSnapshotRecordRow>(
    ...
    const result: PostgresTodosSnapshotPushResult = { records: 0, objectTypes: {} };
  ...
  async pullSnapshot(options: PullPostgresTodosSnapshotOptions = {}): Promise<TodosStorageSnapshot> {
    ...
    const response = await this.client.query<TodosPostgresSnapshotRecordRow>(
...
export function createPostgresTodosSnapshotStore(
  client: TodosPostgresQueryClient,
  options: CreatePostgresTodosSnapshotStoreOptions = {},
): PostgresTodosSnapshotStore {
  return new PostgresTodosSnapshotStore(client, options);
}
...
function assertCanonicalScopedSlugEntry(entry: { type: TodosPostgresSnapshotRecordType; payload: unknown }): void {
...
function entry(type: TodosPostgresSnapshotRecordType, payload: Record<string, unknown>, fallbackUpdatedAt: string) {
  const id = payload["id"];
...
function rowsToSnapshot(rows: TodosPostgresSnapshotRecordRow[]): TodosStorageSnapshot {
