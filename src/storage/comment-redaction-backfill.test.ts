  ...
  const client: TodosPostgresQueryClient = {
    async query<T>(sql: string, values: readonly unknown[] = []) {
      ...
      if (sql.includes("comment-redaction-backfill-scan")) {
        ...
        const selected = rows
          .filter((row) => row.service === service && !row.deleted && row.object_id > String(afterId))
      ...
      if (sql.includes("comment-redaction-backfill-apply")) {
        ...
        const row = rows.find((candidate) =>
          candidate.service === service && candidate.object_id === objectId && !candidate.deleted &&
...
describe("historical Postgres comment redaction backfill", () => {
  test("defaults to a bounded, non-mutating dry run and reports only aggregate counts", async () => {
    ...
    expect(report).toEqual({ dry_run: true, scanned: 2, candidates: 1, updated: 0, conflicts: 0, batches: 2 });
  ...
  test("requires an explicit apply confirmation and is idempotent", async () => {
    ...
    expect(applied).toEqual({ dry_run: false, scanned: 2, candidates: 1, updated: 1, conflicts: 0, batches: 1 });
    ...
    expect(repeated).toEqual({ dry_run: false, scanned: 2, candidates: 0, updated: 0, conflicts: 0, batches: 1 });
  });
  ...
  test("reports compare-and-set conflicts without overwriting a concurrently changed row", async () => {
    ...
    expect(report).toMatchObject({ candidates: 1, updated: 0, conflicts: 1 });
