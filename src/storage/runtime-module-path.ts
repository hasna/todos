...
/** Resolve the single Stage A public runtime owner from source or built code. */
export function resolveTodosPublicRuntimeOwnerModulePath(callerUrl: string): string {
  const caller = new URL(callerUrl);
  if (caller.pathname.endsWith(".ts") || caller.pathname.endsWith(".tsx")) {
    return fileURLToPath(new URL("../stage-a-public-runtime.ts", caller));
  }

  let directory = new URL(".", caller);
  while (directory.pathname !== "/") {
    if (directory.pathname.endsWith("/dist/")) {
      return fileURLToPath(new URL("stage-a-public-runtime.js", directory));
    }
    const parent = new URL("../", directory);
    if (parent.pathname === directory.pathname) break;
    directory = parent;
  ...
  if (new URL(".", caller).pathname.endsWith("/storage/")) {
    return fileURLToPath(new URL("../stage-a-public-runtime.js", caller));
  }
  return fileURLToPath(new URL("./stage-a-public-runtime.js", caller));
}
