...
function buildCapabilities(version: string): TodosCapability[] {
  return [
    ...
    capability(version, {
      ...
      kind: "cli",
      name: "todos events",
      description: "List or tail the local append-only JSONL event stream for agent coordination.",
      tags: ["events", "cli", "jsonl", "local"],
      docsPath: "README.md#local-event-stream",
      stability: "stable",
      inputSchema: {
        type: "object",
        properties: {
    ...
    capability(version, {
      ...
      kind: "mcp",
      name: "tail_events",
      description: "Read local append-only events as JSONL from the MCP server without hosted services.",
      tags: ["mcp", "events", "jsonl", "agents"],
      docsPath: "README.md#local-event-stream",
      stability: "stable",
      inputSchema: {
        type: "object",
