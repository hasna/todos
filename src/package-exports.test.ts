...
describe("package subpath exports", () => {
  ...
  test("provides API contracts, enum values, and generic error codes", () => {
    ...
    expect(manifest.jsonOutputs.contracts.map((contract) => contract.id)).toEqual(
      expect.arrayContaining(["task", "project", "agent", "template", "task_list", "comment", "checkpoint", "dispatch", "audit_history", "local_event", "status_summary", "structured_error", "api_error"]),
    );
