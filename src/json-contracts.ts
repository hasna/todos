...
export const TODOS_JSON_CONTRACTS: TodosJsonObjectContract[] = [
  ...
  contract({
    id: "local_event",
    name: "Local Event",
    description: "Append-only local event stream entry for task, plan, run, comment, dependency, and agent changes.",
    surfaces: ["cli", "mcp", "sdk"],
    stability: "stable",
    required: {
      sequence: field("integer", "Monotonic local sequence number."),
      id: idField,
      event_type: field("string", "Event type, e.g. task.created or run.heartbeat."),
      entity_type: field("string", "Primary entity type."),
      entity_id: nullableIdField,
