...
describe("task list and completion OpenAPI contract", () => {
  ...
  test("generated SDK listTasks exposes total and all supported query fields", async () => {
    ...
    expect(new URL(calls[0]!).searchParams.toString()).toBe(
      "status=pending&priority=high&project_id=project&parent_id=parent&include_subtasks=true&plan_id=plan&task_list_id=list&assigned_to=assignee&agent_id=agent&include_archived=true&limit=1&offset=6",
    );
  });
});
