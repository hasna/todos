...
describe("Stage A external artifact provenance", () => {
  test("the evidence builder binds source, build, inventory, modes, smokes, and archive", () => {
    ...
    const verifierSource = readFileSync(join(root, "scripts/verify-stage-a-evidence.ts"), "utf8");
    const evidenceSources = `${source}\n${policySource}\n${candidateSource}\n${verifierSource}`;

    expect(source).toContain("31988ba7a1ca3d42f50cb2fab894a3581f8e568f");
    expect(source).toContain("const SCHEMA_VERSION = 5");
    expect(source).toContain("collectCanonicalCandidate(repository, BASE_REF)");
    expect(candidateSource).toContain("todos-stage-a-candidate-identity-v6");
    expect(candidateSource).toContain("todos-stage-a-filesystem-closure-v2");
    expect(candidateSource).toContain('"/proc/self/fd/3"');
    expect(verifierSource).toContain("manifest.schema_version === 5");
    for (const field of ["sorted_tracked_paths", "tracked_records", "tracked_inventory"]) {
