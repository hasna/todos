...
describe("Stage A external artifact provenance", () => {
  test("the evidence builder binds source, build, inventory, modes, smokes, and archive", () => {
    ...
    expect(evidenceSources).toContain('"--frozen-lockfile"');
    expect(evidenceSources).toContain('"--ignore-scripts"');
    expect(evidenceSources).toContain('"--backend=copyfile"');
    expect(evidenceSources).toContain('"--linker=hoisted"');
