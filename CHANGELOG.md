...
### Added
- Cross-backend search parity tests (`src/lib/search-parity.pg.test.ts`, gated by `TODOS_TEST_PG_URL`): the same corpus and queries return equivalent ranked results on SQLite (FTS5) and Postgres (tsvector/trigram) — full-text, multi-term AND, quoted phrases, diacritics folding, tag matching, and title-first ranking. Fuzzy/typo matching is Postgres-only and covered separately.
