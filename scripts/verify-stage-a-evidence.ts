...
assertCommandRecordMatchesPolicy(dependencyInstall, DEPENDENCY_INSTALL_POLICY, "offline dependency install");
invariant(
...
invariant(
  credentialScan.gitleaks.stdin === "ignore"
    ...
    && credentialScan.gitleaks.termination === "exit",
...
try {
  ...
  invariant(replay.exitCode === credentialScan.gitleaks.exit_code && existsSync(replayReport), "independent gitleaks replay exit/report mismatch");
  invariant(Buffer.compare(replay.stdout, readEvidenceFile(credentialScan.gitleaks.stdout.path)) === 0, "gitleaks stdout bytes changed during replay");
  invariant(Buffer.compare(replay.stderr, readEvidenceFile(credentialScan.gitleaks.stderr.path)) === 0, "gitleaks stderr bytes changed during replay");
