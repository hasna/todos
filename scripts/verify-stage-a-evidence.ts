...
const replaySource = process.argv.includes("--replay-source");
const skipArtifactSmokes = process.argv.includes("--skip-artifact-smokes");
...
function validateIdentity(identity: FileIdentity, dependencyInventory?: Map<string, InventoryEntry>): void {
  ...
  const entry = dependencyInventory.get(dependencyPath);
...
invariant(manifest.schema_version === 3, "unsupported evidence schema");
...
invariant(
  manifest.network_isolation.sandbox_source_path === manifest.network_isolation.sandbox_tool.path
    && manifest.network_isolation.sandbox_source_sha256 === manifest.network_isolation.sandbox_tool.sha256,
  "sandbox launcher is not bound to the bundled bwrap identity",
);
invariant(
  typeof manifest.network_isolation.sandbox_execution_path === "string"
    && isAbsolute(manifest.network_isolation.sandbox_execution_path)
    && manifest.network_isolation.sandbox_execution_path === TRUSTED_VERIFIER_EXECUTION_POLICY.hostBwrap
    && realpathSync(TRUSTED_VERIFIER_EXECUTION_POLICY.hostBwrap) === TRUSTED_VERIFIER_EXECUTION_POLICY.hostBwrap
    && manifest.network_isolation.sandbox_execution_path === manifest.network_isolation.sandbox_tool.source_path
  ...
  "sandbox execution path policy is not canonical and byte-bound",
);
validateIdentity(manifest.network_isolation.sandbox_launch_shell);
invariant(
  typeof manifest.network_isolation.sandbox_launch_shell_path === "string"
    && isAbsolute(manifest.network_isolation.sandbox_launch_shell_path)
    && manifest.network_isolation.sandbox_launch_shell_path === TRUSTED_VERIFIER_EXECUTION_POLICY.hostBash
    && realpathSync(TRUSTED_VERIFIER_EXECUTION_POLICY.hostBash) === TRUSTED_VERIFIER_EXECUTION_POLICY.hostBash
    && manifest.network_isolation.sandbox_launch_shell_path === manifest.network_isolation.sandbox_launch_shell.source_path,
  "sandbox launch shell is not canonical and byte-bound",
);
...
invariant(typeof manifest.tools.actual_bun_version === "string" && manifest.tools.actual_bun_version.length > 0, "actual Bun version is missing");
...
const dependencyEntries = readTar(evidencePath(manifest.dependencies.archive.path), false);
equalInventory(dependencyEntries.map(({ bytes: _bytes, ...entry }) => entry), expectedDependencyInventory, "dependency archive");
invariant(manifest.dependencies.special_member_count === 0 && manifest.dependencies.hardlink_member_count === 0, "dependency archive special-member claim changed");
...
const dependencyInstall = manifest.dependencies.clean_frozen_offline_install as CommandRecord;
invariant(
  dependencyInstall.label === "offline frozen dependency closure"
    && dependencyInstall.exit_code === 0
    && dependencyInstall.expected_exit === 0
    && dependencyInstall.network_isolated === true
    && dependencyInstall.replayable === false
    && dependencyInstall.replay_omission === "offline cache is intentionally excluded; lock bytes, closure inventory, deterministic archive, and extraction are independently verified",
  "offline dependency install evidence contract changed",
);
invariant(JSON.stringify(dependencyInstall.argv) === JSON.stringify([
  "/opt/bin/bun", "install", "--frozen-lockfile", "--ignore-scripts",
  "--backend=copyfile", "--linker=hoisted", "--cache-dir=/cache", "--no-progress",
]), "offline dependency install argv changed");
invariant(
  dependencyInstall.env.BUN_INSTALL_CACHE_DIR === "/cache"
    && dependencyInstall.env.BUN_INSTALL === "/tmp/bun-install"
    && dependencyInstall.env.BUN_TMPDIR === "/tmp"
    && !Object.hasOwn(dependencyInstall.env, "BUN_CONFIG_REGISTRY")
    && dependencyInstall.launch_argv.includes("--unshare-net")
    && dependencyInstall.launch_argv.includes("--unshare-pid")
    && dependencyInstall.launch_argv.some((value, index) => value === "--ro-bind-fd" && dependencyInstall.launch_argv[index + 2] === "/cache")
    && dependencyInstall.launch_argv.some((value, index) => value === "--ro-bind-fd" && dependencyInstall.launch_argv[index + 1] === "4" && dependencyInstall.launch_argv[index + 2] === "/")
    && !dependencyInstall.launch_argv.includes("--ro-bind"),
  "offline dependency install was not cleanly cache-bound in the minimal sandbox",
);
validateOutputComparison(dependencyInstall);
validateIdentity(dependencyInstall.stdout);
validateIdentity(dependencyInstall.stderr);
invariant(dependencyInstall.stdout.size <= MAX_COMMAND_OUTPUT_BYTES && dependencyInstall.stderr.size <= MAX_COMMAND_OUTPUT_BYTES && dependencyInstall.launch_argv.some((value) => value.includes("ulimit -f 65536")), "offline dependency output capture is not bounded regular-file I/O");
for (const identity of [...dependencyInstall.tools, ...dependencyInstall.inputs]) validateIdentity(identity, dependencyMap);
...
function sandboxInvocation(hostWorkspace: string, env: Record<string, string>, argv: string[]): SandboxInvocation {
  ...
  try {
    const shellDescriptor = openExternalFileMatchingIdentity(
      TRUSTED_VERIFIER_EXECUTION_POLICY.hostBash,
      recordedHostBash,
      "canonical Bash launcher",
    );
    ...
    descriptors.push(shellDescriptor, workspaceDescriptor, rootfsDescriptor, runtimeDescriptor, bwrapDescriptor);
    const boundDescriptors = descriptors.slice(1);
...
function runSandbox(hostWorkspace: string, env: Record<string, string>, argv: string[]): { status: number; stdout: Buffer; stderr: Buffer; launchArgv: string[] } {
  assertHostSandboxRuntimeBinding();
  const invocation = sandboxInvocation(hostWorkspace, env, argv);
  const outputRoot = mkdtempSync(join(tmpdir(), "todos-stage-a-command-output-"));
  const stdoutPath = join(outputRoot, "stdout.bin");
  const stderrPath = join(outputRoot, "stderr.bin");
  const stdoutDescriptor = openSync(stdoutPath, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, 0o600);
  const stderrDescriptor = openSync(stderrPath, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, 0o600);
  try {
    let result!: ReturnType<typeof spawnSync>;
    try {
      result = spawnSync(invocation.argv[0]!, invocation.argv.slice(1), {
        encoding: null,
      ...
      for (const descriptor of invocation.descriptors) closeConsumedDescriptor(descriptor);
      assertHostSandboxRuntimeBinding();
    }
    return {
      status: result.status ?? 70,
      stdout: readBoundedRegular(stdoutPath, MAX_COMMAND_OUTPUT_BYTES),
      stderr: readBoundedRegular(stderrPath, MAX_COMMAND_OUTPUT_BYTES),
      launchArgv: invocation.argv,
    };
  } finally {
    closeConsumedDescriptor(stdoutDescriptor);
    closeConsumedDescriptor(stderrDescriptor);
...
const bunVersionResult = runSandbox(verifierRuntimeHome, {
  PATH: "/opt/bin:/bin",
  HOME: "/srv",
  TMPDIR: "/tmp",
  LANG: "C.UTF-8",
...
const untrackedInventory = parseJsonFile<Array<InventoryEntry & { evidence_path: string }>>(manifest.provenance.untracked_inventory);
invariant(untrackedInventory.length <= MAX_INVENTORY_ENTRIES, "untracked inventory bound exceeded");
const untrackedPaths = [...untrackedInventory].sort((left, right) => byteSort(left.path, right.path));
const sortedPathBytes = Buffer.from(`${untrackedPaths.map((entry) => entry.path).join("\0")}${untrackedPaths.length ? "\0" : ""}`);
invariant(Buffer.compare(sortedPathBytes, readEvidenceFile(manifest.provenance.sorted_untracked_paths.path)) === 0, "sorted NUL untracked path stream mismatch");
const canonicalChunks: Buffer[] = [readEvidenceFile(manifest.provenance.binary_diff.path)];
for (const entry of untrackedPaths) {
  ...
  canonicalChunks.push(Buffer.from(`untracked\0${entry.path}\0${entry.sha256}  ${entry.path}\0`));
}
const canonicalInput = Buffer.concat(canonicalChunks);
invariant(Buffer.compare(canonicalInput, readEvidenceFile(manifest.provenance.canonical_digest_input.path)) === 0, "canonical digest input bytes mismatch");
invariant(sha256Bytes(canonicalInput) === manifest.canonical_source_candidate_digest, "canonical source digest mismatch");
invariant(Buffer.compare(Buffer.concat(canonicalChunks.slice(1)), readEvidenceFile(manifest.provenance.untracked_records.path)) === 0, "untracked canonical records mismatch");
...
try {
  ...
  try {
    if (manifest.provenance.binary_diff.size > 0) {
      ...
      const result = runSandbox(reconstructionRoot, {
        PATH: "/opt/bin:/bin",
        HOME: "/srv",
        LANG: "C.UTF-8",
        LC_ALL: "C",
...
validateIdentity(credentialScan.gitleaks.stdout);
validateIdentity(credentialScan.gitleaks.stderr);
invariant(JSON.stringify(credentialScan.gitleaks.expected_exit) === JSON.stringify([0, 1]) && [0, 1].includes(credentialScan.gitleaks.exit_code) && credentialScan.gitleaks.max_archive_depth === 2 && credentialScan.gitleaks.max_decode_depth === 32, "gitleaks execution policy changed");
invariant(
  credentialScan.gitleaks.descriptor_anchored === true
    && Array.isArray(credentialScan.gitleaks.argv)
    && /^\/proc\/[0-9]+\/fd\/[0-9]+$/.test(credentialScan.gitleaks.argv[0])
    && credentialScan.gitleaks.argv[1] === "dir"
    && /^\/proc\/[0-9]+\/fd\/[0-9]+$/.test(credentialScan.gitleaks.argv[2])
    && credentialScan.gitleaks.argv.includes("--report-path")
    && /^\/proc\/[0-9]+\/fd\/[0-9]+\/\.gitleaks-unredacted-report\.json$/.test(credentialScan.gitleaks.argv[credentialScan.gitleaks.argv.indexOf("--report-path") + 1]),
  "recorded gitleaks descriptor binding is invalid",
);
...
for (const finding of credentialScan.gitleaks.findings as Array<{ rule: string; file: string; line: number; value_digest: string; classification: string }>) {
  invariant(typeof finding.rule === "string" && typeof finding.file === "string", "invalid redacted gitleaks finding fields");
  invariant(finding.classification === "synthetic_test_fixture" && Number.isSafeInteger(finding.line) && /^[a-f0-9]{64}$/.test(finding.value_digest), "invalid redacted gitleaks finding");
  invariant(!finding.file.startsWith("/") && !finding.file.includes("\0") && !finding.file.includes("\\"), "gitleaks finding path is unsafe");
}

const gitleaksReplayRoot = mkdtempSync(join(tmpdir(), "todos-stage-a-gitleaks-"));
try {
  const replayReport = join(gitleaksReplayRoot, "report.json");
  const toolDescriptor = openEvidenceRegular(TRUSTED_VERIFIER_EXECUTION_POLICY.bundledGitleaks);
  const provenanceDescriptor = openEvidenceDirectory("provenance");
  let replay!: ReturnType<typeof spawnSync>;
  const provenancePath = `/proc/${process.pid}/fd/${provenanceDescriptor}`;
  const provenanceRealPath = realpathSync(provenancePath);
  try {
    const replayArgv = [
      `/proc/self/fd/${toolDescriptor}`, "dir", provenancePath,
    ...
    replay = spawnSync(replayArgv[0]!, replayArgv.slice(1), {
    ...
    closeSync(toolDescriptor);
    closeSync(provenanceDescriptor);
  }
  invariant(replay.status === credentialScan.gitleaks.exit_code && existsSync(replayReport), "independent gitleaks replay exit/report mismatch");
  invariant(Buffer.compare(Buffer.from(replay.stdout ?? []), readEvidenceFile(credentialScan.gitleaks.stdout.path)) === 0, "gitleaks stdout bytes changed during replay");
  invariant(Buffer.compare(Buffer.from(replay.stderr ?? []), readEvidenceFile(credentialScan.gitleaks.stderr.path)) === 0, "gitleaks stderr bytes changed during replay");
  const reportBytes = readBoundedRegular(replayReport, MAX_STRUCTURED_JSON_BYTES);
  const rawReplayFindings = JSON.parse(reportBytes.toString("utf8")) as Array<{ RuleID?: unknown; File?: unknown; StartLine?: unknown; Secret?: unknown }>;
  invariant(Array.isArray(rawReplayFindings) && rawReplayFindings.length <= 100_000, "replayed gitleaks report is unbounded");
  const provenancePrefixes = [`${provenancePath}${sep}`, `${provenanceRealPath}${sep}`];
  const replayFindings = rawReplayFindings.map((finding) => {
    invariant(typeof finding.RuleID === "string" && typeof finding.File === "string" && Number.isSafeInteger(finding.StartLine) && typeof finding.Secret === "string" && finding.Secret.length > 0, "gitleaks replay emitted an invalid finding");
    const provenancePrefix = provenancePrefixes.find((prefix) => (finding.File as string).startsWith(prefix));
    invariant(provenancePrefix, "gitleaks replay escaped descriptor-anchored provenance");
    const file = (finding.File as string).slice(provenancePrefix.length).split(sep).join("/");
    ...
    invariant(syntheticFixture, "gitleaks replay found an unapproved credential-like value");
    return {
      rule: finding.RuleID,
      file,
      line: finding.StartLine as number,
      value_digest: sha256Bytes(finding.Secret),
...
invariant(typeof manifest.network_isolation.parent_namespace === "string" && /^net:\[[0-9]+\]$/.test(manifest.network_isolation.parent_namespace), "invalid parent network namespace identity");
invariant(typeof manifest.network_isolation.child_namespace === "string" && /^net:\[[0-9]+\]$/.test(manifest.network_isolation.child_namespace), "invalid child network namespace identity");
invariant(manifest.network_isolation.parent_namespace !== manifest.network_isolation.child_namespace, "network namespace was not isolated");
...
for (const [position, record] of allRecords.entries()) {
  ...
  invariant(record.launch_argv[0] === manifest.network_isolation.sandbox_launch_shell_path && record.launch_argv.includes(manifest.network_isolation.sandbox_execution_path) && record.launch_argv.some((value, index) => value === "--ro-bind-fd" && record.launch_argv[index + 1] === "4" && record.launch_argv[index + 2] === "/") && !record.launch_argv.includes("--ro-bind"), `sandbox is not descriptor-bound to the minimal rootfs: ${record.label}`);
...
const requiredReplay = manifest.required_replay_indices as number[];
invariant(JSON.stringify(requiredReplay) === JSON.stringify(Array.from({ length: commandRecords.length }, (_value, index) => index)), "required replay index set does not cover every source execution");
const requiredReplayLabels = new Map<number, string>([
  [0, "generated SDK stability"],
  [1, "install-free build"],
  [2, "declaration build"],
  [3, "source root cold/warm import"],
  [4, "source contracts cold/warm import"],
  [5, "source MCP public cold/warm import"],
  [6, "source MCP constructor cold/warm import"],
  [7, "source MCP HTTP cold/warm import"],
  [8, "source registry cold/warm import"],
  [9, "source SDK cold/warm import"],
  [10, "source storage cold/warm import"],
  [11, "source direct storage cold/warm import"],
  [12, "built root cold/warm import"],
  [13, "built contracts cold/warm import"],
  [14, "built MCP public cold/warm import"],
  [15, "built MCP constructor cold/warm import"],
  [16, "built MCP HTTP cold/warm import"],
  [17, "built registry cold/warm import"],
  [18, "built SDK cold/warm import"],
  [19, "built storage cold/warm import"],
  [20, "built direct storage cold/warm import"],
  [21, "source CLI metadata"],
  [22, "source CLI containment"],
  [23, "source MCP containment"],
  [24, "source server containment"],
  [25, "built CLI metadata"],
  [26, "built CLI containment"],
  [27, "built MCP containment"],
  [28, "built server containment"],
]);
for (const index of requiredReplay) {
  invariant(commandRecords[index]?.replayable === true, `required claim is not replayable: ${index}`);
  invariant(commandRecords[index]?.label === requiredReplayLabels.get(index), `required replay label changed at ${index}`);
}
...
function replayRecord(record: CommandRecord, hostWorkspace: string): void {
  validateOutputComparison(record);
  const result = runSandbox(hostWorkspace, record.env, record.argv);
  invariant(result.status === record.expected_exit, `replay exit mismatch: ${record.label}: ${result.stderr.toString("utf8").slice(0, 1_000)}`);
  ...
  if (record.output_comparison.mode === "exact-bytes") {
    invariant(Buffer.compare(actualStdout, expectedStdout) === 0, `replay stdout bytes differ: ${record.label}`);
    invariant(Buffer.compare(actualStderr, expectedStderr) === 0, `replay stderr bytes differ: ${record.label}`);
  } else {
    invariant(normalizeRecordedOutput(actualStdout, record.output_comparison.rules) === normalizeRecordedOutput(expectedStdout, record.output_comparison.rules), `normalized replay stdout differs: ${record.label}`);
    invariant(normalizeRecordedOutput(actualStderr, record.output_comparison.rules) === normalizeRecordedOutput(expectedStderr, record.output_comparison.rules), `normalized replay stderr differs: ${record.label}`);
  }
  const combined = `${actualStdout.toString("utf8")}\n${actualStderr.toString("utf8")}`;
  invariant(combined.split("HOSTED_AUTHORITY_UNAVAILABLE").length - 1 === record.expected_authority_floor_occurrences, `replay authority floor mismatch: ${record.label}`);
  invariant(!combined.includes("STAGE_A_IMPORT_TRIPWIRE") && !combined.includes("STAGE_A_ENTRYPOINT_TRIPWIRE"), `replay reached tripwire: ${record.label}`);
}
...
replayRecord(networkProbe, verifierRuntimeHome);
if (!skipArtifactSmokes) {
  ...
  try {
    ...
    for (const record of artifactCommandRecords) replayRecord(record, extractionRoot);
  } finally {
...
if (replaySource) {
  ...
  try {
    ...
    for (const index of requiredReplay) replayRecord(commandRecords[index]!, sourceReplayRoot);
...
process.stdout.write(`${JSON.stringify({
  ...
  replayed_source_claims: replaySource ? requiredReplay.length : 0,
