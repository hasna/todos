...
invariant(manifest.schema_version === 5, "unsupported evidence schema");
...
for (const identity of [
  ...
  manifest.provenance.sorted_tracked_paths,
  manifest.provenance.tracked_records,
...
const trackedInventory = parseJsonFile<CanonicalTrackedIdentity[]>(manifest.provenance.tracked_inventory);
invariant(trackedInventory.length <= MAX_INVENTORY_ENTRIES, "tracked inventory bound exceeded");
...
invariant(
  JSON.stringify(trackedInventory) === JSON.stringify(decodedCandidate.tracked),
  "versioned tracked identity inventory differs from the canonical candidate",
);
invariant(
  Buffer.compare(
    canonicalSortedPathBytes(trackedInventory.map((entry) => entry.path)),
    readEvidenceFile(manifest.provenance.sorted_tracked_paths.path),
  ) === 0,
  "sorted NUL tracked path stream mismatch",
);
invariant(
  Buffer.compare(canonicalTrackedRecords(trackedInventory), readEvidenceFile(manifest.provenance.tracked_records.path)) === 0
    && Buffer.compare(decodedCandidate.trackedRecords, readEvidenceFile(manifest.provenance.tracked_records.path)) === 0,
  "versioned tracked canonical records mismatch",
);
const sourceFilesByPath = new Map(sourceBefore.filter((entry) => entry.type === "file").map((entry) => [entry.path, entry]));
for (const entry of trackedInventory) {
  const sourceEntry = sourceFilesByPath.get(entry.path);
  if (entry.type === "absent") {
    invariant(sourceEntry === undefined, `absent tracked identity exists in copied source: ${entry.path}`);
    continue;
  }
  invariant(sourceEntry !== undefined, `tracked evidence identity is absent from copied source: ${entry.path}`);
  invariant(
    sourceEntry.mode === entry.mode && sourceEntry.size === entry.size && sourceEntry.sha256 === entry.sha256,
    `tracked copied-source identity mismatch: ${entry.path}`,
  );
}
