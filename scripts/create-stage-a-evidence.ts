...
import {
  ...
  collectCanonicalCandidate,
  decodeCanonicalCandidateInput,
  readCanonicalTrackedIdentity,
...
async function canonicalCandidate(): Promise<CanonicalCandidate> {
  return collectCanonicalCandidate(repository, BASE_REF);
}
...
const sortedTrackedPathBytes = canonicalSortedPathBytes(sourceCandidateBefore.sortedTrackedPaths);
const sortedTrackedPathsFile = join(provenanceRoot, "tracked-paths-v6.nul");
writeFileSync(sortedTrackedPathsFile, sortedTrackedPathBytes);
const sortedTrackedPathsIdentity = fileIdentity(sortedTrackedPathsFile);
const trackedInventory = sourceCandidateBefore.tracked.map((expectedIdentity) => {
  const source = readCanonicalTrackedIdentity(repositoryDescriptor, expectedIdentity.path);
  if (JSON.stringify(source.identity) !== JSON.stringify(expectedIdentity)) {
    throw new Error(`candidate tracked identity changed before evidence capture: ${expectedIdentity.path}`);
  }
  return expectedIdentity;
});
const trackedInventoryFile = writeJson(join(provenanceRoot, "tracked-identities-v6.json"), trackedInventory);
const trackedRecordsPath = join(provenanceRoot, "tracked-records-v6.bin");
writeFileSync(trackedRecordsPath, sourceCandidateBefore.trackedRecords);
const trackedRecordsIdentity = fileIdentity(trackedRecordsPath);

const sortedUntrackedPathBytes = canonicalSortedPathBytes(sourceCandidateBefore.sortedUntrackedPaths);
const sortedUntrackedPathsFile = join(provenanceRoot, "untracked-paths-v6.nul");
...
const untrackedInventoryFile = writeJson(join(provenanceRoot, "untracked-identities-v6.json"), untrackedInventory);
...
if (!decodedCandidateInput.trackedRecords.equals(sourceCandidateBefore.trackedRecords)) {
  throw new Error("canonical candidate tracked records mismatch");
}
const canonicalInputPath = join(provenanceRoot, "canonical-candidate-v6.bin");
...
const untrackedRecordsPath = join(provenanceRoot, "untracked-records-v6.bin");
...
const isApprovedHighConfidenceCredentialPath = (path: string): boolean => {
  ...
  return [
    ...
    "provenance/canonical-candidate-v6.bin",
    "provenance/tracked-records-v6.bin",
    "provenance/untracked-records-v6.bin",
  ].includes(path);
};
...
const manifest = {
  ...
  provenance: {
    ...
    sorted_tracked_paths: sortedTrackedPathsIdentity,
    tracked_records: trackedRecordsIdentity,
    tracked_inventory: trackedInventoryFile,
