  ...
  for (const key of Reflect.ownKeys(value.prototype)) {
    ...
    if (typeof descriptor.value === "function") {
      ...
      methods.push(
        ...
        + `Reflect.get(loadRuntime().${exportName}.prototype, ${JSON.stringify(key)}), this, [${parameters.forwarded}]); }`,
      );
    }
    if (descriptor.get) {
      methods.push(
  ...
  for (const key of Reflect.ownKeys(value)) {
    ...
    if (typeof descriptor.value === "function") {
      ...
      staticMembers.push(
        ...
        + `assertTodosLocalStorageRole(process.env); assertPublicSqliteBoundaryArguments([${parameters.forwarded}]); return Reflect.apply(`
        + `Reflect.get(loadRuntime().${exportName}, ${JSON.stringify(key)}), this, [${parameters.forwarded}]); }`,
      );
    }
    if (descriptor.get) {
      staticMembers.push(
